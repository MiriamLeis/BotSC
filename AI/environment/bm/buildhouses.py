...
class BuildMarines(StarcraftEnv): 
    ...
    def get_reward(self, action):
        reward = 0
        if len(self._get_number_of_built_building(group_type=self._TERRAN_SUPLY_DEPOT)) > self.actualMarines:
            #reward = self.actualMarines
            reward = len(self._get_number_of_built_building(group_type=self._TERRAN_SUPLY_DEPOT)) - self.actualMarines
            self.actualMarines = len(self._get_number_of_built_building(group_type=self._TERRAN_SUPLY_DEPOT))
